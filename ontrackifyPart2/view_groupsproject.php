...
// Fetch all projects
$sql = "SELECT p.id, p.name, p.semester, p.course_code
        FROM projects p";
                    ...
                    <div class="card-body">
                        ...
                        <form method="POST" action="join_projectguide.php">
                            ...
                            <?php if (in_array($project['id'], $joinedProjects)): ?>
                                <!-- Remove the Join button if already joined -->
                                <!-- <button type="submit" class="btn btn-success" disabled>Joined</button> -->
                            <?php else: ?>
                                <button type="submit" class="btn btn-success">Join</button>
                            <?php endif; ?>
                        </form>
                        <!-- Replace View Groups with Evaluate -->
                        <a href="guide_evaluation.php?project_id=<?php echo $project['id']; ?>" class="btn btn-primary mt-2">Evaluate</a>
                    </div>
                </div>
            </div>
        <?php endforeach; ?>
